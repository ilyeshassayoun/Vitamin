...
export async function POST(request: Request) {
  ...
  if (!item || item.status !== 'completed')
    return Response.json(
      { error: 'Reviews open only after both sides complete the interaction.' },
      { status: 403 },
    );
  const effortRating = Number(body.effortRating);
  const outcome = ['yes', 'no', 'ongoing', 'not_applicable'].includes(
    String(body.outcome),
  )
    ? String(body.outcome)
    : 'not_applicable';
  const direction =
    typeof body.potentialDirection === 'string'
      ? body.potentialDirection.trim().slice(0, 1500)
      : null;
  if (!Number.isInteger(effortRating) || effortRating < 1 || effortRating > 5)
    return Response.json(
      { error: 'Choose an effort rating from 1 to 5.' },
      { status: 400 },
    );
  const revieweeId =
  ...
  if (!revieweeId)
    return Response.json(
      { error: 'This demo mentor is not linked to an account yet.' },
      { status: 409 },
    );
  try {
    ...
    if (error instanceof Error && error.message.includes('UNIQUE'))
      return Response.json(
        { error: 'You already submitted this review.' },
        { status: 409 },
      );
    throw error;
  }
  ...
  if ((count?.count ?? 0) >= 2) {
    ...
    await Promise.all([
      notify(
        ...
        'reviews_released',
        'Feedback is ready',
        'Both private reviews are now available.',
      ),
      notify(
  ...
  return Response.json(
    { ok: true, released: (count?.count ?? 0) >= 2 },
    { status: 201 },
  );
}

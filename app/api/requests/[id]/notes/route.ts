...
export async function POST(
  ...
  if (!item || !['accepted', 'active'].includes(String(item.status)))
    return Response.json(
      { error: 'Notes are available during an accepted engagement.' },
      { status: 403 },
    );
  const data = await readJsonObject(request);
  if (!data) return invalidJsonResponse();
  const body =
  ...
  await notify(
    ...
    `${user.displayName} added a note to your conversation.`,
  );
